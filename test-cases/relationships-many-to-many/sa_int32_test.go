...
func TestSAInt32Repository_Create_WithADuplicatePrimaryKeyFails(t *testing.T) {
	...
	for repoType, componentUnderTest := range saInt32ImplementationsToTest() {
		...
		initialBuilder := []*relationships_many_to_many.SAInt32_builder{
			{
				Id:   0,
				Data: "0",
			},
		...
		duplicatesBuilder := []*relationships_many_to_many.SAInt32_builder{
			{
				Id:   0,
				Data: "0",
			},
...
func TestSAInt32Repository_Create_WithANonDuplicatePrimaryKeySucceeds(t *testing.T) {
	...
	for repoType, componentUnderTest := range saInt32ImplementationsToTest() {
		...
		expectedBuilder := []*relationships_many_to_many.SAInt32_builder{
			{
				Id:   0,
				Data: "0",
			},
...
func TestSAInt32Repository_Update_WithLocatablePrimaryKeySucceeds(t *testing.T) {
	...
	for repoType, componentUnderTest := range saInt32ImplementationsToTest() {
		...
		expected := make([]*relationships_many_to_many.SAInt32, 0, len(initial))
		for _, saint32 := range initial {
			expected = append(
				expected,
				relationships_many_to_many.SAInt32_builder{
					Id:   saint32.GetId(),
					Data: "UPDATED",
				}.Build(),
			)
...
func TestSAInt32Repository_Delete_WithLocatablePrimaryKeySucceeds(t *testing.T) {
	...
	testCases := map[string]struct {
		initial          []*relationships_many_to_many.SAInt32_builder
		deleteExpression expressions.Expression
		expected         []*relationships_many_to_many.SAInt32_builder
	}{
		"using primary key": {
			initial: []*relationships_many_to_many.SAInt32_builder{
				{
			...
			deleteExpression: expressions.NewOr(
				expressions.NewEquals(
					expressions.NewIdentifier(relationships_many_to_many.Saint32_Id_Field),
		...
		"using non-prime attributes": {
			...
			deleteExpression: expressions.NewOr(
				expressions.NewEquals(
					expressions.NewIdentifier(relationships_many_to_many.Saint32_Data_Field),
					expressions.NewScalar("2"),
				),
				expressions.NewEquals(
					expressions.NewIdentifier(relationships_many_to_many.Saint32_Data_Field),
...
func saInt32Build(in []*relationships_many_to_many.SAInt32_builder) []*relationships_many_to_many.SAInt32 {
	out := make([]*relationships_many_to_many.SAInt32, 0, len(in))
...
func saInt32DefaultCmpOpts() cmp.Options {
	return cmp.Options{
		cmpopts.IgnoreUnexported(relationships_many_to_many.SAInt32{}),
		cmpopts.SortSlices(func(x, y *relationships_many_to_many.SAInt32) bool {
