...
func TestMAAllRepository_Create_WithADuplicatePrimaryKeyFails(t *testing.T) {
	...
	for repoType, componentUnderTest := range maAllImplementationsToTest() {
		...
		initialBuilder := []*relationships_many_to_many.MAAll_builder{
			{
				IdEnum:   relationships_many_to_many.PrimaryKeyEnum_ZERO,
				IdInt32:  0,
				IdInt64:  0,
				IdUint32: 0,
				IdUint64: 0,
			...
			{
				IdEnum:   relationships_many_to_many.PrimaryKeyEnum_ONE,
				IdInt32:  1,
				IdInt64:  1,
				IdUint32: 1,
				IdUint64: 1,
			...
			{
				IdEnum:   relationships_many_to_many.PrimaryKeyEnum_TWO,
				IdInt32:  2,
				IdInt64:  2,
				IdUint32: 2,
				IdUint64: 2,
			...
			{
				IdEnum:   relationships_many_to_many.PrimaryKeyEnum_THREE,
				IdInt32:  3,
				IdInt64:  3,
				IdUint32: 3,
				IdUint64: 3,
		...
		initial := make([]*relationships_many_to_many.MAAll, 0, len(initialBuilder))
		...
		duplicatesBuilder := []*relationships_many_to_many.MAAll_builder{
			{
				IdEnum:   relationships_many_to_many.PrimaryKeyEnum_ZERO,
				IdInt32:  0,
				IdInt64:  0,
		...
		duplicates := make([]*relationships_many_to_many.MAAll, 0, len(duplicatesBuilder))
...
func TestMAAllRepository_Create_WithANonDuplicatePrimaryKeySucceeds(t *testing.T) {
	...
	for repoType, componentUnderTest := range maAllImplementationsToTest() {
		...
		expectedBuilder := []*relationships_many_to_many.MAAll_builder{
			{
				IdEnum:   relationships_many_to_many.PrimaryKeyEnum_ZERO,
				IdInt32:  0,
				IdInt64:  0,
		...
		expected := make([]*relationships_many_to_many.MAAll, 0, len(expectedBuilder))
...
func TestMAAllRepository_Update_WithLocatablePrimaryKeySucceeds(t *testing.T) {
	...
	for repoType, componentUnderTest := range maAllImplementationsToTest() {
		...
		expected := make([]*relationships_many_to_many.MAAll, 0, len(initial))
		for _, maall := range initial {
			expected = append(
				expected,
				relationships_many_to_many.MAAll_builder{
...
func TestMAAllRepository_Delete_WithLocatablePrimaryKeySucceeds(t *testing.T) {
	...
	testCases := map[string]struct {
		initial          []*relationships_many_to_many.MAAll_builder
		deleteExpression expressions.Expression
		expected         []*relationships_many_to_many.MAAll_builder
	}{
		"using primary key": {
			initial: []*relationships_many_to_many.MAAll_builder{
				{
			...
			deleteExpression: expressions.NewOr(
				expressions.NewEquals(
					expressions.NewIdentifier(relationships_many_to_many.Maall_IdEnum_Field),
					expressions.NewScalar(relationships_many_to_many.PrimaryKeyEnum_TWO),
				),
				expressions.NewEquals(
					expressions.NewIdentifier(relationships_many_to_many.Maall_IdEnum_Field),
					expressions.NewScalar(relationships_many_to_many.PrimaryKeyEnum_THREE),
				),
			),
			expected: []*relationships_many_to_many.MAAll_builder{
				{
		...
		"using non-prime attributes": {
			...
			deleteExpression: expressions.NewOr(
				expressions.NewEquals(
					expressions.NewIdentifier(relationships_many_to_many.Maall_Data_Field),
	...
	for testDesc, testCase := range testCases {
		for repoType, componentUnderTest := range maAllImplementationsToTest() {
			...
			initial := make([]*relationships_many_to_many.MAAll, 0, len(testCase.initial))
...
func maAllBuild(in []*relationships_many_to_many.MAAll_builder) []*relationships_many_to_many.MAAll {
	out := make([]*relationships_many_to_many.MAAll, 0, len(in))
...
func maAllDefaultCmpOpts() cmp.Options {
	return cmp.Options{
		cmpopts.IgnoreUnexported(relationships_many_to_many.MAAll{}),
		cmpopts.SortSlices(func(x, y *relationships_many_to_many.MAAll) bool {
